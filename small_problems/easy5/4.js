...
// Create a newArr based on originalArray.
// Check if value is already in newAr
// If it is, return the value

function findDup (originalArray) {
 let newArr = [];
 for (let idx = 0; idx < originalArray.length; idx += 1) {
  if (newArr.includes(originalArray[idx])) {
    return originalArray[idx];
  }
  newArr.push(originalArray[idx]);
 }
