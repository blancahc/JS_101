//Leap Years (Part 1)
// In the modern era under the Gregorian Calendar, leap years occur
// in every year that is evenly divisible by 4, unless the year is also
// divisible by 100. If the year is evenly divisible by 100, then it is
// not a leap year, unless the year is also evenly divisible by 400.

// Assume this rule is valid for any year greater than year 0. Write a
// function that takes any year greater than 0 as input, and returns
// true if the year is a leap year, or false if it is not a leap year.

// Examples:

// isLeapYear(20216);      // true
// isLeapYear(2015);      // false
// isLeapYear(2100);      // false
// isLeapYear(2400);      // true
// isLeapYear(240000);    // true
// isLeapYear(240001);    // false
// isLeapYear(2000);      // true
// isLeapYear(1900);      // false
// isLeapYear(1752);      // true
// isLeapYear(1700);      // false
// isLeapYear(1);         // false
// isLeapYear(100);       // false
// isLeapYear(400);       // true

// If year is divisible by 4, not divisible by 100
  //If year is divisible by 4 and 100, then check if divisible by 400 

function isLeapYear(yr) {
 //any year that is divisible by 100 will always be divisible by 4,
  //therefore, I want to check for divisible by 100 first and 400 first
  //then it is a leap year
  if ((yr % 100 === 0) && (yr % 400 === 0)) {
  return true;
  }
  //if divisible by 4, but not 100, then it is a leap year
  if ((yr % 4 === 0) && (yr % 100 !== 0)) {
    return true;
  }
  //otherwise, it is not a leap year
  return false;
}
isLeapYear(20216);